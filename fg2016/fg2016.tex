...
We will now try to use this calculus to do some semantics. Here is the
tectogrammar of our syntax in an abstract categorial grammar
...
meaning of~\eqref{ex:deixis}, we have an algebraic expression having as
operands the propositions $\app{\obj{love}}{\obj{m}}{x}$ for all possible
$x : \iota$. In order to get a single proposition which is to be seen as
the truth-conditional meaning of the sentence and which can be evaluated in
a model, we will need to fix the speaker. We will do so by defining an

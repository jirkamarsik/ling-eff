...
\cite{shan2005linguistic,van2010computational},\footnote{Side effects are
...
algebra. We can then combine the signatures of the algebras by summing them
up. The free algebra of the resulting signature will serve as a universal
representation format for the set of all terms built from any of the source
algebras and closed on substitution. Then, we will build modular
interpreters that will give meanings to the operators of the algebras in
terms of individuals, truth-values and functions.
...
operator $\hsbind$, called bind, replaces all the $\alpha$-typed leaves of
a $\FF_E(\alpha)$-typed expression with $\FF_E(\beta)$-typed
...
following combinators (variations on the application operator) which we
will make heavy use of in Section~\ref{sec:phenomena}.

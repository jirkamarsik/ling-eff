...
module LinguisticEffectsSel where
...
data Sel v = SelR AnaphoraTag [Entity] (Entity -> v)
             deriving (Typeable, Functor)

selR :: (Member Sel r) => AnaphoraTag -> [Entity] -> Eff r Entity
selR tag ents = send_req (SelR tag ents)

trySel :: (Member Choose r) => Eff (Sel :> r) a -> Eff r a
trySel m = loop (admin m) where
  loop (Val x) = return x
  loop (E u) = handle_relay u loop handler
  handler (SelR tag ents k) = choose ents >>= (loop . k)

runRef :: (Member Sel r) => [Entity] -> Eff (Ref :> r) Bool -> Eff r Bool
runRef rs m = loop rs (admin m) where
  ...
  handler rs (FreshR k) = runRef rs $ exists' (\x -> do xv <- x
                                                        loop (xv:rs) (k xv))
  handler rs (FetchR tag k) = do selected_ref <- selR tag rs
                                 loop rs (k selected_ref)
  handler rs (AssertC prop k) = fmap (&& prop) $ loop rs (k ())

enter :: (Member Sel r, Member Ref r) => [Entity] -> Eff r Bool -> Eff r Bool
enter rs m = loop rs (admin m) where
  loop rs (Val x) = return x
  loop rs (E u) = interpose u (loop rs) (handler rs)
  handler rs (FreshR k) = exists' (\x -> do xv <- x
                                            loop (xv:rs) (k xv))
  handler rs (FetchR tag k) = do selected_ref <- selR tag rs `mplus'` fetchR tag
                                 loop rs (k selected_ref)
  handler rs (AssertC prop k) = fmap (&& prop) $ loop rs (k ())
...
exists' :: (Member Sel r, Member Ref r) => EffTr r ((Entity -> Bool) -> Bool)
exists' predM = (liftM or) (mapM (enter [] . predM . return) domain')

forall' :: (Member Sel r, Member Ref r) => EffTr r ((Entity -> Bool) -> Bool)

...
module LinguisticEffectsDoneRight where
...
instance Show (Formula a) where
  ...
  show (Abs var body) = "(lambda (" ++ show var ++ ") " ++ show body ++ ")"
...
getR :: (Member Ref r) => Eff r Context
getR = send_req GetR

assertC :: (Member Ref r) => Formula Bool -> Eff r ()
assertC prop = send_req (AssertC prop)

fetchR :: (Member Ref r, Member Choose r)
       => AnaphoraTag -> Eff r (Formula Entity)
fetchR tag = do rs <- getR
                choose rs

data Scope a
 
scopeOver :: (Member Scope r) => Eff r (Formula Entity -> Eff r (Formula Bool))
scopeOver = undefined

runRef :: (Member Fresh r)
...
runRef rs m = loop rs (admin m) where
  ...
  handler rs (FreshR k) = exists' (\x -> do xv <- x
                                            loop (xv:rs) (k xv))
  handler rs (GetR k) = loop rs (k rs)
  handler rs (AssertC prop k) = (return prop) `and'` loop rs (k ())

enter :: (Member Fresh r, Member Ref r)
...
it :: (Member Ref r, Member Choose r) => EffTr r Entity 
it = fetchR It

he :: (Member Ref r, Member Choose r) => EffTr r Entity 
he = fetchR He

she :: (Member Ref r, Member Choose r) => EffTr r Entity 
she = fetchR She

...
;; Below is a mechanization of the λ-banana calculus defined
;; in 'Effects and Handlers in Natural Language'. The mechanization
;; can be consulted to verify the computations done in
;; the dissertation and see a formalized definition of
;; the calculus.
...
;; =====================
;; 
;; These are the terms of the λ-banana calculus, as defined
;; in Section 1.2 of the dissertation.
(define-language BANANA
  (e ::= x
         ...
         (η e)
         (OP e (λ (x) e))
         ;; Since we cannot (easily) change the delimiters
         ;; from parentheses to banana brackets, we employ
         ;; a different notation in this implementation.
         (with (OP e) ... (η e) handle e)
         ;; DrRacket does not have a convenient shortcut for a
         ;; cherry symbol and so we use \flat.
         (♭ e)
         (C e))
...
;; We then extend the set of terms with the constructions
;; for sum types and product types from Section 1.4
;; of the dissertation.
...
;; Finally, we add the ambiguity operator that we introduced
;; in Subsection 7.3.4 of the dissertation. Since semicolon
;; is used by Racket to indicate comments, we use || as the symbol.
...
;; (subst e x e_new) is the result of substituting e_new for all
;; the free occurrences of x in e (i.e. e[x := e_new])
(define-metafunction BANANA+SPA
  subst : e x e -> e
  [(subst x x e_new)
   e_new]
...
;; We can now define the reduction relation of our calculus.
;; This follows closely the definitions given in Chapter 1
;; of the dissertation.
(define reduce
  (compatible-closure 
    (reduction-relation
      ...
      (--> (with (OP_1 e_1) ... (OP_2 e_2) (OP_3 e_3) ... (η e_p) handle (OP_2 e_arg (λ (x) e_m)))
           ((e_2 e_arg) (λ (x_f) (with (OP_1 e_1) ... (OP_2 e_2) (OP_3 e_3) ... (η e_p) handle (subst e_m x x_f))))
           (side-condition (term (no-match OP_2 (OP_1 ...))))
           (fresh x_f)
           "handle-OP")
      (--> (with (OP_i e_i) ... (η e_p) handle (OP e_arg (λ (x) e_m)))
           (OP e_arg (λ (x_f) (with (OP_i e_i) ... (η e_p) handle (subst e_m x x_f))))
...
;; ===================
;;
;; When computing the normal forms of the terms in our dissertation,
;; we often assume that the anaphora resolution operators sel_he,
;; sel_she, sel_it and selP choose some specific individual from
;; the context, or in the case of selP, accept that the context
;; does not contain any suitable individual and reduce to some
;; value which signals this. We will want to use our mechanization
;; to reduce λ-banana terms to readable truth-conditions
;; and so we include reduction rules that implement a very basic
;; form of anaphora resolution into the reduction relation
;; of our calculus.
...
;; (sel-prop p c i)
;; iff i is an individual accessible in the context c and which,
;; according to the context c, satisfies the predicate p
(define-judgment-form
  BANANA+SPAC
  #:mode (sel-prop I I O)
...
;; reduce-more extends the reduction relation 'reduce of our calculus.
;; It adds rules that concatenate contexts and look within them
  ...
  (compatible-closure
    (extend-reduction-relation reduce
      ...
      (--> ((selP e_property) e_context)
           ...
           (side-condition (not (judgment-holds (sel-prop e_property e_context e_referent))))
           "selP-not-found"))
    BANANA+SPAC
    e))
...
;; ==================
;;
;; Defined below are utility functions that allow us to normalize
;; and pretty-print terms.

;; We enable caching of metafunctions in hopes to squeeze out
;; some extra performance.
(caching-enabled? #t)
(set-cache-size! 1024)

;; normalize uses the apply-reduction-relation of Redex to find
;; a normal form for a λ-banana term.
...
;; map-children maps a function over the immediate subterms of
;; a term. In Haskell parlance, this is the Functor instance
;; for λ-banana terms.
...
;; it. Normalization often makes a term smaller, which increases
;; the performance of identifying redexes and therefore leads
;; to faster normalization times.
(define (normalize-bottom-up rel t)
  (normalize rel (map-children (λ (x) (normalize-bottom-up rel x)) t)))


;; simplify-logic is a reduction relation that implements some
;; simple logical rules. Their point is to sanitize the logical
;; formulas generated by our system by, e.g., decoding the logical
;; operator ∀, ⇒ and ∨.
...
;; λ-binders to quantifiers. Since the result of this translation
;; is no longer a valid term in our calculus, we have to define
;; a new notion of context closure.
  ...
  (context-closure
    (reduction-relation
      ...
      (--> ((c_pred any_1 ...) any_2)
           ...
           ;; logical predicates in the dissertation to the convention
           ;; of using symbols ending with * in this mechanization.
           (side-condition (string-suffix? (symbol->string (term c_pred)) "*"))))
    BANANA+SPAL
    context))

;; compute-truth-conditions combines all the steps necessary to go
;; from a λ-banana term which encodes the meaning of a sentence
;; to human-readable truth-conditions of that sentence.
...
;; term? verifies whether a Redex term is a λ-banana term
;; (i.e. does it conform to the grammar of this formalization).
(define term? (term-match/single BANANA+SPA [e #t] [_ #f]))

;; (check-term t) is a unit test that checks whether is a valid
;; term of our calculus.
(define (check-term t)
  (test-equal (term? t) #t))

;; define-checked-term extends Redex's define-term by checking
;; that every defined term conforms to the grammar. 
(define-syntax-rule (define-checked-term name body)
  (begin
    (define-term name body)
    (check-term (term name))))

;; find-broken-form is a debugging utility. If a term is not accepted
;; by the grammar as a valid verm, find-broken-form helps us to find
;; the minimal subterm which is mistyped. term-children is an auxiliary
;; function that allows us to recover the immediate subterms of
;; a mistyped term.
...
;; ==================
;;
;; This part mirrors Section 1.6 of the dissertation.
;; It introduces syntactic shortcuts, combinators, that we will
;; make heavy use 
...
;; The op! syntax lets uses an operation with the trivial
;; continuation (λ x. η x).
(define-metafunction BANANA+SPA
  [(! OP)
   (λ (x) (OP x (λ (y) (η y))))]
  [(! OP e)
...
;; We functionalize OP, i.e. we turn the OP expression constructor
;; into a function expression. Also known as a generic effect.
(define-metafunction BANANA+SPA
  gen : OP -> e
  [(gen OP)
   (λ (x) (λ (k) (OP x (λ (y) (k y)))))])

;; This construction lets us omit the η clause when writing
;; a handler. The default clause η: (λ x. η x) is used.
(define-metafunction BANANA+SPA
  with-η : (OP e) ... handle e -> e
  [(with-η (OP e_h) ... handle e_arg)
   (with (OP e_h) ... (η (λ (x) (η x))) handle e_arg)])

;; We functionalize handlers. In λ-banana, this corresponds
;; to writing a handler without giving its argument.
(define-metafunction BANANA+SPA
  handler : (OP e) ... (η e) -> e
  [(handler (OP e_h) ... (η e_p))
   (λ (x) (with (OP e_h) ... (η e_p) handle x))])

;; We combine the two last abstractions to define a functionalized
;; handler expression with the default η clause.
(define-metafunction BANANA+SPA
  handler-η : (OP e) ... -> e
  [(handler-η (OP e_h) ...)
...
;; The next three definitions concern the expression of Boolean
;; values using sums (Subsection 1.5.4 in the dissertation).
;; We define constants for true...
...
;; We define function application for cases when the function
;; is provided by a computation.
(define-metafunction BANANA+SPA
  ≪· : e e -> e
  [(≪· e_f e_x)
   (>>= e_f (λ (f) (η (f e_x))))])

;; We also define function application when the argument is
...
;; Finally, we define function application for when both function
;; and argument are the results of computations. This is the <*>
;; binary operator of applicative functors.
(define-metafunction BANANA+SPA
  ≪·≫ : e e -> e
  [(≪·≫ e_f e_x)
   (>>= e_f (λ (f) (>>= e_x (λ (x) (η (f x))))))])

;; We will extend the above idea of applying an operation
;; to computations that yield the operands to other operations.
;; We introduce a macro that, given an operator 'op, defines
;; the extended versions '≪op, 'op≫ and '≪op≫.
...
;; When defining the open handler for dynamics (box), we will
;; make use of the following two combinators, introduced in
;; Subsection 7.3.1 of the dissertation.
(define-metafunction BANANA+SPA
  <<<· : e e -> e
  [(<<<· e_f e_x)
   (>>= e_f (λ (f) (f e_x)))])
...
;; ========
;;
;; The rest of the program will cover the final grammar presented
;; in Chapter 8. We start by first regrouping the definitions
;; of all the handlers.

;; This is the box handler for dynamics, based on its presentation
;; in Section 8.1. Note that the INTRODUCE operation has been
;; decomposed into FRESH and PUSH as in Subsection 8.5.1.
...
;; We have replaced INTRODUCE by FRESH and PUSH and so we express
;; INTRODUCE in terms of FRESH and PUSH.
(define-metafunction BANANA+SPA
  INTRODUCE : e e -> e
  [(INTRODUCE e_u e_k)
     (FRESH e_u (λ (x)
...
;; The empty handler (Section 8.1) evaluates the discourse in
;; an empty anaphoric context.
(define-checked-term empty
  (handler-η (GET (λ (_) (λ (k) (k nil))))))

;; Next, we turn to presupposition (Section 8.2). We have
;; the accommodate handler that accommodates presuppositions
;; by introducing new discourse referents.
;; Note that the predicate P is assumed to be a computation
;; with DRT effects (GET, FRESH, PUSH, ASSERT) and not just
;; a plain predicate. This is to licence anaphoric binding
;; from definite descriptions, as in Subsection 8.6.2.
(define-checked-term accommodate
  (handler-η (PRESUPPOSE (λ (P) (λ (k)
               (INTRODUCE ★ (λ (x) (>>= (P x) (λ (_) (k x))))))))))

;; We will need to make a non-deterministic choice when trying
;; to accommodate a presupposition at different levels. The choose
;; expression constructor, which corresponds to the + operator
;; in the dissertation (Section 8.2), gives us a convenient syntax
;; for the choice operation AMB : 1 → 2.
(define-metafunction BANANA+SPA
  choose : e e -> e
  [(choose e_1 e_2)
   (AMB ★ (λ (b) (ifte b e_1 e_2)))])

;; The maybeAccommodate handler uses choose to consider both
;; projecting the presupposition and accommodating it.
(define-checked-term maybeAccommodate
  (handler-η (PRESUPPOSE (λ (P) (λ (k)
               (choose (PRESUPPOSE P (λ (x) (k x)))
                       (INTRODUCE ★ (λ (x) (>>= (P x) (λ (_) (k x)))))))))))

;; The find combinator is of the same type as (! PRESUPPOSE).
;; It tries to look for the missing entity in the context. If it
;; cannot be found, it projects the presupposition.
;; Note that this find is the one from Subsection 8.6.2, which expects
;; dynamic predicates as arguments and uses ♭ ∘ empty ∘ box to make
;; them static.
...
;; The useFind handler tries resolving the presuppositions within
;; its arguments using find.
(define-checked-term useFind
  (handler-η (PRESUPPOSE (λ (P) (λ (k) (>>= (find P) k))))))

;; maybeAccommodate introduces ambiguity via the AMB operator.
;; The search handler resolves the ambiguity by choosing which
;; of the two possibilities to pursue. In the dissertation,
;; we make this choice based on whether or not the computations fail.
;; In this mechanization, we leave the ambiguity operator
;; unresolved.
(define-checked-term search
  (handler-η (AMB (λ (_) (λ (k) ((k true) || (k false)))))))

;; We incorporate the possibility to accommodate a presupposition
;; in every DRS on the projection line by introducing maybeAccommodate
;; to the box handler. We also add useFind so that presuppositions
;; can be (preferably) found within the context without having to
;; be accommodated.
;; We will still need to use the original box handler and so we name
;; this one dbox (in Chapter 7 of the dissertation, we used 'box
;; with a bar on top).
(define-checked-term dbox
  (∘ box maybeAccommodate useFind))

;; When extending our treatment of presuppositions to complex
;; definite descriptions (e.g. when dealing with restrictive relative
;; clauses) in Subsection 8.6.2, we introduced a series of
;; handlers which are applied to an effectful predicate before
;; it is projected as a presupposition.

;; inTheContext evaluates its argument in a given anaphoric context.
;; This is used so that even when the predicate ends up being
;; interpreted in some higher context, it still has access to
;; both the referents available in its context and those introduced
;; within the predicate.
(define-checked-term inTheContext
  (λ (e) (handler-η (GET (λ (_) (λ (k) (GET ★ (λ (e_) (k ((++ e) e_))))))))))

;; separateDynamics delays the execution of all dynamic effects
;; (GET, INTRODUCE (i.e. FRESH and PUSH), and ASSERT).
...
;; packageProperty takes an effectful property (e.g. the meaning
;; of a complex noun), evaluates some of its effects and produces
;; a property whose only effects are dynamic.
(define-checked-term packageProperty
  (λ (P) (GET ★ (λ (e)
         (C (λ (x) (separateDynamics ((inTheContext e) (maybeAccommodate (SI (P x)))))))))))

;; The next two handlers are the handlers for conventional implicature
;; from Section 8.3. The asImplicature handler translates ASSERT
;; to IMPLICATE and INTRODUCE (i.e. FRESH and PUSH) to INTRODUCE_I
;; (i.e. FRESH_I and PUSH_I).
(define-checked-term asImplicature
  (handler-η (FRESH (gen FRESH_I))
             (PUSH (gen PUSH_I))
             (ASSERT (gen IMPLICATE))))

;; The withImplicatures handler reintegrates conventional implicatures
;; into the layer of the main asserted meaning by reversing
;; the translation done by asImplicature.
(define-checked-term withImplicatures
  (handler-η (FRESH_I (gen FRESH))
             (PUSH_I (gen PUSH))
             (IMPLICATE (gen ASSERT))))

;; withSpeaker is the handler for first-person pronouns from
;; Section 8.4.
...
;; Finally, we can compose all of the handlers to get an interpreter
;; that maps any computation in our fragment to a proposition.
...
;; =============
;;
;; This section introduces the logical operators that we will be
;; using in our grammar. These are based on de Groote and Lebedeva's
;; TTDL. Their λ-banana definitions can be found in Section 8.1
;; of the dissertation.
...
;; =======
;;
;; What follows are λ-banana terms which are the interpretations
;; of the lexical items that make up our grammar. This grammar combines
;; all of the phenomena discussed in Chapter 8 of the dissertation.
...
;; common-noun : N
;; Common nouns all have the same kind of meaning and so we define
;; a macro to facilitate the population of the lexicon.
...
;; indef : N → NP
;; This is the semantics of the indefinite article. In the dissertation,
;; we call this constructor 'a. Here we give it a longer name so as
;; to avoid confusion with the variable 'a.
...
;; nil-disc : D
;; The empty discourse. In the dissertation, it is called 'nil.
;; Here we avoid this name because it is already used for the empty
;; context.
(define-checked-term nil-disc
  (η ★))

;; The following entries describe the presupposition triggers.
;; They are based on the extension to the base grammar proposed
;; in Section 8.6.2.
...
(define-checked-term said_ds
  (λ (C_) (λ (S) (SI (>>= S (λ (s) (>>= (·≫ say* (≪·≫ S ((top s) C_))) (! ASSERT))))))))
...
;; The following is a helper combinator for the lexical entry
;; of quantifiers. (trace x) is a term that evaluates to x but also
...
;; We also add some adjectives and copulas to the grammar to be able
;; to cover the examples seen in Chapter 7.

;; angry : ADJ
(define-checked-term angry
  angry*)

;; frightened : ADJ
(define-checked-term frightened
  frightened*)

;; frightened : ADJ
(define-checked-term cheap
  cheap*)
...
;; evaluated. The examples are taken from the dissertation
;; and can therefore be used to verify the calculations that are
;; done "on paper" in the dissertation.

;; My best friend, who owns a dog, said it loves everyone.
;; 
;; (normalize-piece-by-piece reduce-more example-final)
;; '(η (∃ (x8) ((best-friend* x8 s) ∧
;;     (∃ (x1) ((dog* x1) ∧ ((own* x8 x1) ∧

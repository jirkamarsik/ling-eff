...
;; resolve-left reduces all the alternative expression (e_1 || e_2)
;; to their left (by default preferred) alternatives e_1
(define resolve-left
  (letrec ([resolve-left
            (λ (t) (resolve-left-at-top (map-children resolve-left t)))]
           [resolve-left-at-top
            (term-match/single BANANA+SPA
              [(e_1 || e_2)
               (term e_1)]
...
;; resolve-blocked takes a normalized term and simplifies (e_1 || e_2)
;; by removing stuck alternatives (i.e. computations that contain
;; unresolved uses of the C operator)
(define resolve-blocked
  (term-match/single BANANA+SPA
    [(C (λ (x) e))
     (raise 'BLOCKED)]
    [(e_1 || e_2)
     (let* ([blocked? (λ (x) (equal? x 'BLOCKED))]
            [left (with-handlers ([blocked? identity])
                    (resolve-blocked (term e_1)))]
            [right (with-handlers ([blocked? identity])
                     (resolve-blocked (term e_2)))])
       (cond [(and (blocked? left) (blocked? right)) (raise 'BLOCKED)]
             [(blocked? left) right]
             [(blocked? right) left]
             [else (term (,left || ,right))]))]
    [e
     (map-children resolve-blocked (term e))]))
...
(define-metafunction BANANA+SPA
  ...
  [(∘ e)
   e]
  [(∘ e_f e_g)
   ,(term-let ([x_f (variable-not-in (term (e_f e_g)) 'x)])
     (term (λ (x_f) (e_f (e_g x_f)))))]
  [(∘ e_1 e_2 e_more ...)
   (∘ (∘ e_1 e_2) e_more ...)])
...
;; (compute-truth-conditions (term example-socrates-hypotheses))
...
;; (compute-truth-conditions (term example-socrates-conclusion))
...
;; John said "Mary loves me". (Subsection 6.2.1)
;; (compute-truth-conditions (term example-direct-speech))
...
;; (compute-truth-conditions (term example-john-man))
...
;; (compute-truth-conditions (term example-secondary-crossover))
...
;; (compute-truth-conditions (term example-bad-crossover))

...
$\operatorname{raise_\alpha}$. In the definitions below, variables such as
$a$ and $b$ range over atomic types.

\begin{align*}
  \operatorname{raise_a}(x) &= \eta\ x \\
  \operatorname{raise_{a \to \beta}}(f) &= \lambda X.\ X \hsbind (\lambda
  x. \operatorname{raise_\beta}(f\ x))
\end{align*}

We could now produce a lexicon $\mathcal{L}_1$ such that for every $c :
\alpha \in \Sigma^a_0$, we have $\mathcal{L}_1(c) =
\operatorname{raise_\alpha}(\mathcal{L}_0(c))$. There is just one hitch, we
...
$\operatorname{raise^R_\alpha}$ that simulates right-to-left evaluation
instead of left-to-right. We start by unfolding the recursive definition of
$\operatorname{raise_\alpha}$ and then modifying it.

\begin{align*}
  \operatorname{raise_{a_1 \to\ \ldots\ \to a_n \to b}}(f) &= \lambda X_1 \ldots X_n.\ X_1 \hsbind (\lambda x_1.\ \ldots\ X_n \hsbind (\lambda x_n.\ \eta\ (f\ x_1 \ldots x_n))) \\
  \operatorname{raise^R_{a_1 \to\ \ldots\ \to a_n \to b}}(f) &= \lambda X_1 \ldots X_n.\ X_n \hsbind (\lambda x_n.\ \ldots\ X_1 \hsbind (\lambda x_1.\ \eta\ (f\ x_1 \ldots x_n))) \\
\end{align*}

We have $\operatorname{raise^R_{NP \to NP \to S}}(\abs{loves}) = \lambda X
...
\operatorname{raise^R_\alpha}(\mathcal{L}_0(c))$.
...
\begin{align*}
  ...
  \dnot &= \operatorname{raise_{o \to o}}(\lnot) = \lambda X.\ X \hsbind (\lambda x.\ \eta\ (\lnot x)) \\
  \dand &= \operatorname{raise_{o \to o \to o}}(\land) = \lambda X Y.\ X \hsbind (\lambda x.\ Y \hsbind (\lambda y.\ \eta\ (x \land y))) \\
...
\begin{align*}
  ...
  SI &: \mathcal{F}(o) \to \mathcal{F}(o) \\
  SI &= [\mathcal{H}\ (SCOPE\ (\lambda c k.\ c\ k))] \\
  \lex{loves}{\lambda X Y.\ SI\ (\mathcal{L}(\abs{loves})\ X\ Y)} \\
  \lex{knows}{\lambda X Y.\ SI\ (\mathcal{L}(\abs{knows})\ X\ Y)} \\
  \lex{says}{\lambda S X.\ SI\ (\mathcal{L}(\abs{says})\ S\ X)} \\
\end{align*}
...
term, a handler called $SI$. Since tensed clauses form scope islands, we
insert $SI$ into the existing lexical entries for tensed verbs.

If we had a more fine-grained grammar which would provide a separate
lexical entry for the finite verb morpheme, we could simply add the $SI$
handler there without having to change all verbs.
...
case, $SI$ would be inserted into the semantics of every such rule which is
to be considered a scope island.

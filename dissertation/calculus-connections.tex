...
calculus' equational theory is preserved\footnote{Such a one-sided property
  ...
  characterization of this kind of simulation? Check out Plotkin's seminal
  paper on Call-by-value, call-by-name and the lambda-calculus.}, i.e.\ $M
= N$ in $\lambda_\shifto$ implies $\sem{M} = \sem{N}$ in $\banana{\lambda}$
(where $X = Y$ is to be read as $X \ttoffrom Y$).

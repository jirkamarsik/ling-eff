Having developed the $\calc$ calculus in Part~\ref{part:calculus}, we will
...
\item[S5] If $\delta \in P_{IV / T}$\footnote{i.e.\ $\delta \in P_{TV}$} and
  $\beta \in P_{T}$, then $F_5(\delta, \beta) \in P_{IV}$, where $F_5(\delta,
  \beta) = \delta \beta$ if $\beta$ does not have the form $\textbf{he}_n$, and
  $F_5(\delta, \textbf{he}_n) = \delta\ \textbf{him}_n$.
...
\item[S14] If $\alpha \in P_{T}$ and $\phi \in P_{t}$, then
  ...
  \item $\alpha = \textbf{he}_k$, and $F_{10,n}(\alpha, \phi)$ comes from $\phi$
    by replacing all occurrences of $\textbf{he}_n$ or $\textbf{him}_n$ by
    $\textbf{he}_k$ or $\textbf{him}_k$
  \end{itemize}
\end{description}
...
\begin{center}
\makebox[\textwidth][l]{
\begin{tikzpicture}[sibling distance=6mm,level distance=2cm]
\tikzset{every tree node/.style={align=center,anchor=north}}
  \Tree [.{\textbf{every man loves a unicorn}, $F_{10,0}$ \\
...
We have presented the $D$ monad. Now, we will show how it features in
...
\subsection{Example Lexical Entries}
...
The first four lexical entries give semantics to noun phrases. These are

...
In this chapter, we will show the connection between calculi containing
operators for delimited control and our $\banana{\lambda}$
calculus. Throughout the chapter, we will introduce different calculi and
map their terms and reduction relations onto the reduction relation of
$\banana{\lambda}$. For one of the calculi with delimited control, we also
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[align=center,thick]
    ...
    \draw[->,dashed] (lambda) .. controls +(-30:2) and +(190:2) .. (banana) node[midway,above]{(\ref{sec:simulating-cbv})};
...
$\sem{C}[\sem{M}] \to \sem{C}[\sem{N}]$\footnote{We have not defined the
  ...
  with a $[]$ inside so we just extend the $\sem{.}$ for terms with the
  clause $\sem{[]} = []$.}, which is the same as
$\sem{C[M]} \to \sem{C[N]}$.
...
calculus' equational theory is preserved
% \footnote{Such a one-sided property
...
\begin{align*}
  \sem{\ap{\shift}{M}} &= \sem{M} \hsbind (\lam{m}{\ap{\op{\shift0}!}{(\lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})}})}}) \\
...
\begin{definition}
  ...
  \item
    ...
    $E_\omega = \{ \typedop{shift0}{((\delta \to \FF_E(\omega)) \to
      \FF_E(\omega))}{\delta} \}_\delta \uplus E$\footnote{The idea behind
      $E_\omega$ is that $\shift0$ should be polymorphic in
      $\delta$. However, we do not have polymorphism in $\banana{\lambda}$,
      so in this particular case, we will assume we have sufficiently many
      distinct instances of $\shift0$ covering the different types at which
      we want to $\shift$.}
  \end{itemize}

  where $E$ is assumed to be some effect signature such that
  $\shift0 \notin E$\footnote{The calculus can be further extended with
    other effects whose effect signature would be E.}.
\end{definition}
...
\begin{proof}
  ...
  (\lam{m}{\ap{\op{\shift0}!}{(\lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{}
            \circ k)})}})}})$. In Figure~\ref{fig:big-proof-tree-shift}, we
...
\begin{sidewaysfigure}
  ...
  \resizebox{1.1\textwidth}{!}{
    ...
    \RightLabel{[$\circ$]}
    \BinaryInfC{$\sem{\Gamma}, k : \sem{\tau} \to \FF_E(\sem{\gamma}) \vdash \banana{} \circ k : \sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})$}
    \RightLabel{[app]}
    \BinaryInfC{$\sem{\Gamma}, m : (\sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})) \to \cstype(\sem{\gamma}), k : \sem{\tau} \to \FF_E(\sem{\gamma}) \vdash \ap{m}{(\banana{} \circ k)} : \cstype(\sem{\gamma})$}
    \RightLabel{[app]}
    \BinaryInfC{$\sem{\Gamma}, m : (\sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})) \to \cstype(\sem{\gamma}), k : \sem{\tau} \to \FF_E(\sem{\gamma}) \vdash \ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})} : \FF_E(\sem{\gamma})$}
    \RightLabel{[abs]}
    \UnaryInfC{$\sem{\Gamma}, m : (\sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})) \to \cstype(\sem{\gamma}) \vdash \lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})}} : (\sem{\tau} \to \FF_E(\sem{\gamma})) \to \FF_E(\sem{\gamma})$}
    \RightLabel{[$\op{op}!$]}
    \UnaryInfC{$\sem{\Gamma}, m : (\sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})) \to \cstype(\sem{\gamma}) \vdash \ap{\op{\shift0}!}{(\lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})}})} : \cstype(\sem{\tau})$}
    \RightLabel{[abs]}
    \UnaryInfC{$\sem{\Gamma} \vdash \lam{m}{\ap{\op{\shift0}!}{(\lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})}})}} : ((\sem{\tau} \to \FF_{E_{\sem{\delta}}}(\sem{\gamma})) \to \cstype(\sem{\gamma})) \to \cstype(\sem{\tau})$}
    \RightLabel{[$\hsbind$]}
    \BinaryInfC{$\sem{\Gamma} \vdash \sem{M'} \hsbind (\lam{m}{\ap{\op{\shift0}!}{(\lam{k}{\ap{\resetobanana}{(\ap{m}{(\banana{} \circ k)})}})}}) : \cstype(\sem{\tau})$}
    \DisplayProof
  }}

...
[,phantom
 [$\lambda_v$ \\ (\ref{sec:introducing-cbv}),name=lambda
   [$\lambda_\shift$ \\ (\ref{sec:turning-to-shift}),tier=bottom,name=shift]
   [$\lambda_\shifto$ \\ (\ref{sec:introducing-control-operators}),tier=bottom,name=shift0]
 ]
 [$\banana{\lambda}$ \\ (\ref{chap:definitions}),tier=bottom,name=banana]
]
\draw[->,dashed] (lambda) to node[above]{(\ref{sec:simulating-cbv})} (banana);
\draw[->,dashed] (shift0) to node[above]{(\ref{sec:simulating-shift0})} (banana);
\draw[->,dashed] (shift) to node[above]{(\ref{sec:turning-to-shift})}(shift0);
\end{forest}
\end{center}

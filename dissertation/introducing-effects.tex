...
with a discourse referent $x$ while at the same time introducing the
discourse referent $x$ and the condition $\obj{Jones}(x)$ into the DRS.\@
In the next DRS, we evaluate the object by replacing it with $y$ and adding
the discourse referent $y$ and a condition in which (the meaning of) the
noun \emph{Porsche} is applied to $y$.
...
reduce the piece of syntax [$x$ owns $y$] and replace it with an atomic
formula $\obj{own}(x,y)$. We can now add the syntactic analysis of the
second sentence and proceed with computation.
...
discourse referent $u$ which we introduce into the DRS along with the
condition $u = y$.
...
For the object noun we do the same, introducing the discourse referent $v$
and the condition $v = x$. Finally, all that is left to translate the
transitive verb into a binary relation and we get the final DRS
representation.
...
Let us now look at the formulation of the construction rules\footnote{We
  ...
  anaphora resolution in this work.}.
...
The rule for pronouns, $\crpro$, is very similar. In introduces a new kind
of operation, in which the NP being evaluated is asking its context for a
suitable anaphoric referent.
    ...
    \drs{$x_1$ \ldots\ $x_n$}
        {$\gamma_1$ \\
         ...
         $C_\gamma \left[ \leaf{a(n)} \branch{1}{DET} \leaf{N}
           \branch{2}{$NP_{Gen=\beta}$} \leaf{VP$'$} \branch{2}{S} \tree \right]$ \\
         \ldots \\
         $\gamma_m$}\ \right]
  \to_\crid
  C\left[\ 

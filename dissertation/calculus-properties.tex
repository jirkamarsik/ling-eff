...
\begin{corollary}\label{cor:banana-tau-termination}
...
\begin{proof}
  By~\ref{thm:labidts-termination} and~\ref{thm:semantic-labelling}.
\end{proof}

\begin{corollary}\label{cor:intcalculus-termination}
...
\begin{proof}
  By~\ref{cor:idts-termination} and~\ref{cor:banana-tau-termination}.
\end{proof}
...
\begin{lemma}\label{lem:pulling-links}
  ...
  Let $t_1 \to t_2 \to \ldots$ be an infinite reduction chain in $\calc$
  which uses rules from $\intcalc$ infinitely often. Then there exists
  another infinite reduction chain $u_1 \to u_2 \to \ldots$ which also uses
  rules from $\intcalc$ infinitely often and $t_1 \to_\intcalc u_1$.
\end{lemma}

\begin{proof}
  Let $t_k \to t_{k+1}$ be the first link in the chain that uses a rule
  from $\intcalc$. We will prove this by induction on $k$.
...
\begin{proof}
  ...
  be an infinite reduction chain in $\banana{\lambda}$.

  First, we will show that this reduction chain must use rules from
  $\intcalc$ infinitely often. If it would not be the case, then let
  $t_n \to t_{n+1}$ be the last rule in the chain coming from
  $\intcalc$. The remaining chain $t_{n+1} \to t_{n+2} \to \ldots$ would be
  an infinite reduction chain using $\eta$ which is impossible since $\eta$
  is terminating.
  
  Since we have a chain in $\calc$ that uses rules from $\intcalc$
  infinitely often, we can iterate Lemma~\ref{lem:pulling-links} to get an
  ...
  sequence. The existence of such a chain is in contradiction with the
  termination of $\intcalc$.
\end{proof}

\begin{theorem}\label{cor:strong-normalization}

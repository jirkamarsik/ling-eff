...
\item the following equations hold for any $f : C \to D$, $g : B \to C$ and
  $h : A \to B$:
  \begin{align}
    \label{law:cat-associativity}(f \circ g) \circ h &= f \circ (g \circ h) \\
    \label{law:cat-left-identity}\id_D \circ f &= f \\
    \label{law:cat-right-identity}f \circ \id_C &= f
  \end{align}
\end{itemize}
\end{definition}
...
\item the following equations hold:
  \begin{align}
    \label{law:functor-composition}F(f \circ g) &= F(f) \circ F(g) \\
    \label{law:functor-identity}F(\id_A) &= \id_{F(A)}
  \end{align}
\end{itemize}
\end{definition}

...
\begin{proof}
  ...
  \item \boxed{C[M'] \to C[N']}
    ...
    The reduction relation of $\banana{\lambda}$ is defined as the
    syntactic closure of the individual reduction rules. We have covered
    the rules themselves, we now address the syntactic closure. By
    induction hypothesis, we know that the reduction from $M' \to N'$
    preserves types, i.e.\ for any $\Delta$ and $\alpha$ such that
...
\item $\op{op}(t_{\mathrm{p}},[x]t_{\mathrm{c}})$ as
  ...
  $\banana{\lambda}$, where the syntactic closure (see~\ref{sec:reductions}) does
...
rules. As in $\banana{\lambda}$, CRS then proceeds to take a syntactic closure of
...
\item The presheaf that we will care the most about will be the presheaf
  $D$ of denotations,
  $D_\tau(\Gamma) = \{ \sem{M} \mid \Gamma \vdash M : \tau \}$. Every
  element of $D_\tau(\Gamma)$ is a function from $\sem{\Gamma}$ to
  $\sem{\tau}$.
...
\item $M_\Sigma Z$ is the presheaf of IDTS metaterms with signature
...
\item First, we introduce an endofunctor on the category of presheafs in
  ...
  extension of context $\Gamma$ by a variable of type $\tau$. The idea
...
\item The signature of our IDTS, $\Sigma$, induces an endofunctor on the
  ...
  $$
  (\Sigma A)_\gamma = \coprod_{f \in F_{\vec{\alpha_1} \to \beta_1, \ldots, \vec{\alpha_l} \to \beta_l, \gamma}} \prod_{1 \le i \le l} \delta_{\vec{\alpha_i}} A_{\beta_i}
  $$

  In the above, we use the vector notation $\vec{\alpha} \to \beta$ for
  $\alpha_1 \to \ldots \to \alpha_n \to \beta$ and $\delta_{\vec{\alpha}}$
...
We will now show that our denotational semantics forms a
$V+\Sigma$-algebra, where $(V+\Sigma)(A) = V + \Sigma(A)$. The carrier will
be the presheaf $D$ of $\banana{\lambda}$ denotations. Now we need to give
an interpretation to variables and to every function symbol defined in the
signature $\Sigma$. This interpretation must be given as an
$\alpha : V + \Sigma D \to D$, meaning that the interpretation of every
function symbol must be compositional. Every value from
$(\Sigma D)_\gamma(\Gamma)$ is some function symbol of result type $\gamma$
together with the denotations of all of the symbol's arguments. These
denotations will be functions from $\sem{\Gamma}$ to $\sem{\beta_i}$ where
$\beta_i$ is the type of the $i$-th argument of the function
symbol. Furthermore, if the symbol binds some variable $x : \alpha_i$
inside one of its arguments, the denotation will be expecting
$\sem{\Gamma, x : \alpha_i}$ instead.

The interpretations that we have given in~\ref{ssec:denotational-semantics}
are exactly of this form. For example, the line defining the denotation of
the expression $\app{\op{op}}{M_\petitp}{(\lam{x}{M_\petitc})}$ can be
transformed into an interpretation for the function symbol $\op{op}$ the
following way:

\begin{align*}
  & \sem{\app{\op{op}}{M_\petitp}{(\lam{x}{M_\petitc})}}(e) =
    \op{op}(\sem{M_\petitp}(e), \lam{X}{(\sem{M_\petitc}({\subst{e}{x}{X}}))}) \\
  & \op{op}^\AAA(D_\petitp, D_\petitc)(e) =
    \op{op}(D_\petitp(e), \lam{X}{D_\petitc({\subst{e}{x}{X}})})
\end{align*}

Therefore, the compositional denotational semantics
from~\ref{ssec:denotational-semantics} gives us a $V+\Sigma$-algebra $\DD$
with carrier $D$.
...
\begin{definition}
  An arrow $f : A_1 \times \ldots \times A_n \to B$ in $\SetC^\ContextC$ is
  \demph{weakly monotonic} if for all $\Gamma$ and
  $a_1, b_1 \in A_1(\Gamma)$, \ldots, $a_n, b_n \in A_n(\Gamma)$ with
  $a_k \ge_{A_k(\Gamma)} b_k$ for some $k$ and $a_j = b_j$ for all
  $j \neq k$, we have that
  $f(\Gamma)(a_1, \ldots, a_n) \ge_{B(\Gamma)} f(\Gamma)(b_1, \ldots,
  b_n)$.
\end{definition}
...
We will now take our $V+\Sigma$-algebra $\DD$ and equip its carrier $D$
with a partial order $\ge_D$ such that $\DD$ is weakly
monotonic. $\ge_{D_\tau(\Gamma)}$ needs to be a partial order on the set
$D_\tau(\Gamma)$, the elements of which are functions from $\sem{\Gamma}$
to $\sem{\tau}$. We will define $\ge_{D_\tau(\Gamma)}$ along with a partial
order $\ge_{\sem{\tau}}$ by induction:

...
\begin{definition}
  ...
  presheaf $Z$ to the presheaf $A$ such that $\phi = ! \circ \theta$,
  where:
...
\begin{definition}
  ...
  $\phi = ! \circ \theta$. The \demph{labelling map}
  ...
  \begin{align*}
    ...
    &= f^{\angles{!\theta^*(t_1), \ldots, !\theta^*(t_n)}^f_\Gamma}([\vec{x_1}] \phi^\Label_{\beta_i,(\Gamma,\vec{x_1} :
...
than an IDTS term. The term-generated assignment $\phi = ! \circ \theta$ is
only used when labelling a function symbol. The IDTS valuation $\theta$ is
used to replace the metavariables in all of the arguments with some
specific terms and the resulting IDTS terms are then interpreted in our
algebra $\TT$ using $!$ (which simply turns them into $\banana{\lambda}$
...
Finally, we have managed to prove termination for $\labidts$, which in turn
implies termination for $\idts$ by Theorem~\ref{thm:semantic-labelling},
which in turn entails termination for $\banana{\lambda}_{-\eta}$ by
Corollary~\ref{cor:banana-tau-termination}, our $\banana{\lambda}$ calculus

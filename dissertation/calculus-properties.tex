...
arguments. However, when defining a denotational semantics for
$\banana{\lambda}$, we had made a formal argument showing that the
arguments to these operations are in fact decreasing. We will therefore
make use of a technique which will allows us to incorporate this semantic
insight into the IDTS so that the General Schema will be able to recognize
the decreasing nature of the arguments.
...
by Makoto Hamana in~\cite{hamana2007higher}. In defining the denotational
semantics of $\banana{\lambda}$ in~\ref{ssec:denotational-semantics}, we
have given an ordering to the denotations in $\sem{\FF_E(\gamma)}$ that
justified the correctness of the recursive definition of $\CC$ and
$\banana{}$. Since that definition mimicked the one present in the
reduction rules, we would like to transfer this result to a proof of
termination for $\banana{\lambda}$.

The idea behind the semantic labelling technique is to label function
symbols with the denotations of their arguments. Every rule will be
replaced by all the possible labellings. Where before we had two 

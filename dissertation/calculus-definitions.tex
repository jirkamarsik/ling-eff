...
\begin{definition}
  ...
  \item if $M \syntclos{\sim} M'$, then $\etaE{M} \syntclos{\sim}
    \etaE{M'}$
  \item if $M_{\mathrm{p}} \syntclos{\sim} M'_{\mathrm{p}}$, then
    $\app{\op{op}}{M_{\mathrm{p}}}{(\lam{x}{M_\mathrm{c}})} \syntclos{\sim}
    \app{\op{op}}{M'_{\mathrm{p}}}{(\lam{x}{M_\mathrm{c}})}$
  \item if $M_{\mathrm{c}} \syntclos{\sim} M'_{\mathrm{c}}$, then
    $\app{\op{op}}{M_{\mathrm{p}}}{(\lam{x}{M_\mathrm{c}})} \syntclos{\sim}
    \app{\op{op}}{M_{\mathrm{p}}}{(\lam{x}{M'_\mathrm{c}})}$
  \item if $M_i \syntclos{\sim} M_i'$, then $\banana{\onto{\op{op}_1}
    M_1,\ \dots,\ \onto{\op{op}_i} M_i,\ \dots,\ \onto{\op{op}_n}
    M_n,\ \onto{\eta}{M_\eta}}{N} \syntclos{\sim} \banana{\onto{\op{op}_1}
    M_1,\ \dots,\ \onto{\op{op}_i} M_i',\ \dots,\ \onto{\op{op}_n}
    M_n,\ \onto{\eta} M_\eta}{N}$
  \item if $M_\eta \syntclos{\sim} M_\eta'$, then
    $\ap{\cibanana}{N} \syntclos{\sim}
     \banana{(\onto{\op{op}_i}{M_i})_{i \in I},\ \onto{\eta}{M_\eta'}}{N}$
  \item if $N \syntclos{\sim} N'$, then $\ap{\cibanana}{N} \syntclos{\sim}
    \ap{\cibanana}{N'}$
  \item if $M \syntclos{\sim} M'$, then $\ap{\cherry}{M} \syntclos{\sim}
    \ap{\cherry}{M'}$ 
  \item if $M \syntclos{\sim} M'$, then $\ap{\CC}{M} \syntclos{\sim}
    \ap{\CC}{M'}$ 
  \end{itemize}
\end{definition}

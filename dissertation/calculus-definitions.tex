...
\begin{figure}
  \begin{tabular}{lr}
  $\ap{(\lam{x}{M})}{N} \to$ & rule $\beta_\to$ \\
  $\subst{M}{x}{N}$ & \\
  \\
  $\lam{x}{\ap{M}{x}} \to$ & rule $\eta_\to$ \\
...
First off, we have the $\beta_\to$ and $\eta_\to$ rules. By no
...
\begin{align*}
  ...
  &\to_{\beta_\to} \ap{\ap{\op{op}}{x}}{\eta} \hsbind k \\
...
have a clause for every operator appear within). Since all effects are
handled and none are forwarded, the codomain of the handler can be
...
$$
\cibbanana = \cherry \circ \banana{\onto{\op{op}_i}{(\lam{x k}{\ap{\eta}{(\ap{\ap{M_i}{x}}{(\cherry \circ k)})}})},\ \onto{\eta}{(\eta \circ M_\eta)}}
$$

As we can see, that only material added by the closed handler brackets are
...
$\FF_\emptyset(\alpha)$. Rather than looking at the definition, the idea of
a closed handler is better conveyed by a giving its typing rule. The
following rule will be proven sound in Chapter~\ref{chap:properties}:

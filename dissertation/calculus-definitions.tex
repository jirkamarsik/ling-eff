...
\item First of all, it acts as a ``free'' variable over effect
  ...
  that $E_1 \subseteq E_2$\footnote{This relation is an instance of
    subtyping and a more robust treatment will be outlined in
    Section~\ref{sec:subtyping}.}.
...
\begin{figure}
  \begin{tabular}{p{0.01\textwidth} p{0.5\textwidth} p{0.01\textwidth} p{0.48\textwidth}}
  \vspace{0pt}
  $M = $
  &
  \vspace{0pt}
  ...
  \Tree [.$\lambda$
    [.\node[draw]{$\op{op}_1(M_1)$};
      [.\node[draw,circle]{$A_1$}; ]
      [.\node[draw]{$\op{op}_2(M_2)$};
        [.\node[draw,circle]{$A_2$}; ] ] ]
    [.\node[draw]{$\op{op}_1(M_1)$};
      [.\node[draw,circle]{$B_1$}; ]
      [.\node[draw]{$\op{op}_2(M_2)$};
  ...
  \Tree [.\node[draw]{$\op{op}_1(M_1)$};
    [.\node[draw,circle,inner sep=0pt,anchor=center]{
  ...
  \end{tikzpicture}
  \end{tabular}
  \caption[Example of applying the $\CC$
  operator.]{\label{fig:c-illustration}
    Example of applying the $\CC$ operator to a term. Here, $M$ is a
function of some two-value type. It maps one value to the left subtree of
$\lambda$ (with leaves $A_i$) and the other value to the right subtree
(leaves $B_i$). Both subtrees correspond to computations, in which a box is
an operation and a circle is an atomic expression (i.e.\ a return
value). Furthermore, $\op{op}_1$ has a two-value output type and
$\op{op}_2$ has a one-value output type.

    Since the operations $\op{op}_1$ and $\op{op}_2$ and their arguments
    $M_1$ and $M_2$ are the same in both subtrees, and thus independent of
    the value passed to the $\lambda$, we can apply the $\CC$ operator. The
    $\CC$ pulls this common structure out of the $\lambda$ and gives us a
    computation that produces a function.}
\end{figure}
...
\begin{definition}
  ...
  % \footnote{\TODO{Maybe add a small note for readers not aware of the
  %     capture problem? I do not expect people to be unfamiliar with it, but
  %     I do spend a few pages talking about the trouble with substitutions
  %     and variables so it might be worth it to have a small intro.}}
...
\begin{definition}
  \demph{Evaluation contexts} are terms with a hole (written as $[]$)
...
\begin{definition}
  $N$ is a \demph{subterm} of $M$ if there exists an evaluation context $C$
  such that $M = C[N]$.
\end{definition}
...
\begin{definition}
  ...
  % \footnote{\TODO{Should I prove that such a relation is uniquely
  %     determined?}}
...
$\to_{\xi_n} \circ \dots \circ \to_{\xi_1}$. The \emph{reduction relation}
$\to$ of $\banana{\lambda}$ is the union of the $\to_\xi$ relations for
every reduction rule $\xi$. We will also use the symbol $\tto$ to stand for
the reflexive-transitive closure $\to^*$ of $\to$.
...
\begin{figure}
  ...
  \begin{subfigure}{.5\textwidth}
   \begin{prooftree}
    \AxiomC{$\Gamma \vdash M : \alpha \times \beta$}
    \RightLabel{[$\pi_1$]}
    \UnaryInfC{$\Gamma \vdash \ap{\pi_1}{M} : \alpha$}
   \end{prooftree}
  \end{subfigure}
  \begin{subfigure}{.5\textwidth}
   \begin{prooftree}
    \AxiomC{$\Gamma \vdash M : \alpha \times \beta$}
    \RightLabel{[$\pi_2$]}
    \UnaryInfC{$\Gamma \vdash \ap{\pi_2}{M} : \beta$}
   \end{prooftree}
  \end{subfigure}
  ...
  \begin{subfigure}{.5\textwidth}
   \begin{prooftree}
    \AxiomC{$\Gamma \vdash M : \alpha$}
    \RightLabel{[$\inl$]}
    \UnaryInfC{$\Gamma \vdash \ap{\inl}{M} : \alpha + \beta$}
   \end{prooftree}
  \end{subfigure}
  \begin{subfigure}{.5\textwidth}
   \begin{prooftree}
    \AxiomC{$\Gamma \vdash M : \beta$}
    \RightLabel{[$\inr$]}
    \UnaryInfC{$\Gamma \vdash \ap{\inr}{M} : \alpha + \beta$}
   \end{prooftree}
  \end{subfigure}
  ...
  \begin{prooftree}
    ...
    \TrinaryInfC{$\Gamma \vdash \case{M}{x}{N_{\mathrm{l}}}{y}{N_{\mathrm{r}}} : \gamma$}
  \end{prooftree}
  \begin{prooftree}
...
&= \ap{\cherry}{(\ap{\banana{(\onto{\op{op}_i}{(\lam{x k}{\ap{\eta}{(\ap{\ap{M_i}{x}}{(\cherry \circ k)})}})})_{i \in I}}}{N})} \\
 &= \ap{\cherry}{(\ap{\banana{(\onto{\op{op}_i}{(\lam{x k}{\ap{\eta}{(\ap{\ap{M_i}{x}}{(\cherry \circ k)})}})})_{i \in I},\ \onto{\eta}{(\lam{x}{\etaE{x}})}}}{N})} \\
 &= \ap{\cherry}{(\ap{\banana{(\onto{\op{op}_i}{(\lam{x k}{\ap{\eta}{(\ap{\ap{M_i}{x}}{(\cherry \circ k)})}})})_{i \in I},\ \onto{\eta}{(\lam{x}{\etaE{(\ap{(\lam{x}{x})}{x})}})}}}{N})} \\

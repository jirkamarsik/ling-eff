...
\footnotetext{After adding deixis, the type of $\TOP$ became
  ...
  type $\FF_E(1) \to \FF_{\{\op{speaker}\}}(o)$. This means will not be
  obliged to provide a referent for the speaker. On the other hand, the
  resulting computation might use the $\op{speaker}$ effect if the sentence
  is indexical.}
...
\subsubsection{Sketching a Solution --- Presupposing Dynamic Propositions}
...
$\typedop{presuppose}{(\iota \to \FF_{E*}(1))}{\iota}$ because then
$E^*$ is not well-defined. For similar reasons, $E^*$ cannot contain
$\typedop{scope}{((\iota \to \FF_{E'}(1)) \to \FF_{E'}(1))}{\iota}$,
...
Now our intention is to handle or evaluate the effects of the noun in the
...
\begin{align*}
  ...
  \packageProperty &= \lam{P}{\begin{aligned}[t]
      &\app{\op{get}}{\star}{(\lam{e}{ \\
      &\ap{\CC}{(\lam{x}{\ap{\splitDRT}{(\app{\inTheContext}{e}{(\ap{\SI}{(\ap{\maybeAccommodate}{(\ap{P}{x})})})})}})}})}
    \end{aligned}} \\
  \\
  \lex{the}{\lam{N}{\begin{aligned}[t]
  ...
  \lex{poss}{\lam{X N}{\begin{aligned}[t]
      &X \hsbind (\lam{x}{ \\
      &\ap{\packageProperty}{(\lam{y}{\app{\op{push}}{y}{(\lam{\_}{\ap{N}{y} \dand \ap{\op{assert}!}{(\app{\obj{own}}{x}{y})}})}})} \hsbind (\lam{N'}{ \\
      &\ap{\op{presuppose}!}{N'}})})
    \end{aligned}}} \\
  \lex{children-of}{\lam{X}{\begin{aligned}[t]
      &\ap{\packageProperty}{(\lam{y}{(\ap{\obj{children}}{y} \apr X) \hsbind \op{assert}!})} \hsbind (\lam{N'}{ \\
      &\ap{\op{presuppose}!}{N'}})
    \end{aligned}}} \\
  &\vdots
\end{align*}

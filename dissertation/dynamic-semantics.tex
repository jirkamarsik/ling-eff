...
TTDL~\cite{lebedeva2012expression} employs an exception handler that allows
for accommodation at every dynamic negation, which is the operation with
which all DRSs in TTDL are constructed. We can achieve the same result in
our $\banana{\lambda}$ analysis by including the $\maybeAccommodate$ in the
$\DBOX$ handler:
...
verb \emph{think} uses boxes, then we get as denotation a nondeterministic
computation that yields the three reading mentioned above.

\begin{exe}
  \ex \label{ex:german-wash} Do Germans?
\end{exe}

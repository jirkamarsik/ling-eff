...
dynamics in natural language: Kamp's DRT~\cite{kamp1993discourse}
(Section~\ref{sec:drt}) and de Groote's
TTDL~\cite{de2006towards,lebedeva2012expression}
(Section~\ref{sec:ttdl}). We will build a $\banana{\lambda}$ analysis of
dynamics and link it to both theories, as a side effect showing how DRT
links to TTDL.\@ The analysis we will present can be motivated on the
grounds of either DRT or TTDL.\@ We can look at the monad at the core of
TTDL and devise operations that let us perform interesting things within
...
termination (w.r.t.\ $\poorcalc$), as we will show below, and therefore
they also preserve strong normalization.
...
\begin{notation}
  The \demph{$\calc_{-\CC}^;$ calculus} is the $\poorcalc$ calculus with
  the $M; N$ construction and its reduction rule, $\to_;$.
\end{notation}
...
\begin{proof}
  We consider $\poorcalc$ with the $M; N$ construction and the following
  two reduction rules.
  ...
  This calculus, which we will call $\calc_{-\CC}^{;\to}$, is not confluent
  but, as we will show below, it is terminating. Since the reduction
  relation of $\calc_{-\CC}^;$ is a subset of the reduction relation of
  $\calc_{-\CC}^{;\to}$, then the reduction relation of $\calc_{-\CC}^;$
  will turn out to be terminating as well.

  For every type $\alpha$, we add a binary function symbol $(;_\alpha)$ of
  type $\alpha \to \alpha \to \alpha$ and typed versions of the two
  reduction rules above to the IDTS $\idts$ from
  Section~\ref{sec:termination}, forming a new IDTS
  $\calc^{;\to}_\tau$. Since we have added no constructor symbols and the
  right-hand sides of the new reduction rules use no function symbols (and
  therefore are not recursive), we still validate the General Schema and
  Theorem~\ref{thm:idts-termination} holds for
  $\calc^{;\to}_\tau$. Following the proof of
  Theorem~\ref{thm:termination}, we know that $\calc_{-\CC}^{;\to}$
  terminates.
\end{proof}
...
\begin{theorem}
  \demph{Strong normalization of $\calc_{-\CC}^;$}

  $\calc_{-\CC}^;$ is strongly normalizing, i.e.\ there are no infinite
  reduction chains in $\calc_{-\CC}^;$ and all maximal reduction chains
  originating in a $\calc_{-\CC}^;$ term $M$ terminate in the same term,

...
\begin{align*}
  ...
  &\tto \ap{\BOX}{(\begin{aligned}[t]
     ...
     &\etaE{\star}})}})})}})})
     \end{aligned}} \\
  &\rightsquigarrow \begin{aligned}[t]
     &\app{\op{get}}{\star}{(\lam{e}{ \\
     &(\casenl{\selP(\lam{y}{\obj{car}(y) \land \obj{own}(\sel(e), y)}, e)}
       {x}{\etaE{x}}
       {\_}{\ap{\op{presuppose}!}{(\lam{y}{\obj{car}(y) \land \obj{own}(\sel(e), y)})}}) \hsbind (\lam{y}{ \\
...
$\rightsquigarrow$. To finish computing $\dnot \sem{S_2}$, we take
...
(see~\ref{ssec:truth-conditions-as-side-effects}), de Groote and Kanazawa's

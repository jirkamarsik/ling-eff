...
level, a handler catches these exceptions and accomodates these
...
the exception, it accomodates the presupposition, rewinds the dynamic state
and evaluates the sentence again, this time with a context which now
...
projection line\footnote{A \emph{projection line} is a path in the
  ...
  tree~\cite{van1992presupposition}.}. We might then end up accomodating
...
$$
\dnot A = \app{\op{get}}{\star}{(\lam{e}{\op{assert}! \apr (\notr (\app{\BOX}{A}{e}))})}
$$

or, without the $\apr$ and $\notr$ syntactic sugar:
...
$\FF_{E_\DRT}(1)$ into computations in $\FF_{\{\typedop{get}{1}{\gamma}\}}(o)$.

\begin{align*}
  \BOX :\ &\FF_{E \uplus E_\DRT}(1) \to \FF_{E \uplus \{\typedop{get}{1}{\gamma}\}}(o) \\
  \BOX = \lam{A}{(\ap{\banana{\ 

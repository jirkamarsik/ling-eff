...
module LinguisticEffectsObjectGQEventsExtr where
...
data Extr v = ExtrR (Formula Entity -> v)
              deriving (Typeable, Functor)

extrR :: (Member Extr r) => Eff r (Formula Entity)
extrR = send_req ExtrR

data Fail v = FailR
              deriving (Typeable, Functor)

failR :: (Member Fail r) => Eff r a
failR = send_req (\_ -> FailR)
...
handleExtr :: (Member Extr r, Member Fail r) =>
              Eff r (Formula a) -> Eff r (Formula (Entity -> a))
handleExtr m = loop False (admin m) where
  loop False (Val x) = failR
  loop True (Val x) = 
  loop isUsed (E u) = interpose u (loop isUsed) (handler isUsed)
  handler False (ExtrR k) = 
  handler True (ExtrR k) = ExtrR 
